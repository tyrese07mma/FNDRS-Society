...
export function FilterSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Sheet
      ...
      onClose={onClose}
      title="Match filters"
      subtitle="Narrow Smart Match to the people you need right now."
      footer={
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <Button title="Reset" variant="secondary" style={{ flex: 1 }} onPress={() => setDraft(DEFAULT_FILTERS)} />
          <Button
            title="Show matches"
            style={{ flex: 2 }}
            onPress={() => {
    ...
    >
      <Group title="ROLE">
        {ROLES.map((r) => (
          <Chip key={r} label={ROLE_LABEL[r]} size="sm" selected={draft.roles.includes(r)} onPress={() => setDraft((d) => ({ ...d, roles: toggle(d.roles, r) }))} />
        ))}
      </Group>
      <Group title="STAGE">
        {STAGES.map((s) => (
          <Chip key={s} label={STAGE_LABEL[s]} size="sm" selected={draft.stages.includes(s)} onPress={() => setDraft((d) => ({ ...d, stages: toggle(d.stages, s) }))} />
        ))}
      </Group>
      <Group title="INDUSTRY">
        {INDUSTRIES.map((i) => (
          <Chip key={i} label={i} size="sm" selected={draft.industries.includes(i)} onPress={() => setDraft((d) => ({ ...d, industries: toggle(d.industries, i) }))} />
        ))}
      </Group>
      <View style={{ gap: 10 }}>
        <Text variant="label" color="textSubtle">MINIMUM FIT</Text>
